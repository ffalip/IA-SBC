...
(deffunction ask-integer-question (?question)
    ...
    (while (not (integerp ?answer)) do
        (printout t "Resposata invŕlida. Siusplau entra un valor enter." crlf)
        (printout t ?question)
        (bind ?answer (read)))
    ?answer)
...
(defrule determina-PressioSangMax
    =>
    (bind ?answer (ask-integer-question "Quina és la teva pressió sanguínia mŕxima? (mmHg) " ))
...
(defrule determina-impacte-caloric
   =>
    (bind ?answer (ask-question "Quin impacte calňric diries que te la teva dieta actual? (baix, normal, alt) " baix normal alt))
...
(defrule mal-esquena
    =>
    (bind ?answer (ask-question "Quant de mal et fa l'esquena del 0 (mínim) al 3 (mŕxim): " 0 1 2 3))
    (assert (fa-mal esq ?answer)))

(defrule mal-bracos
    =>
    (bind ?answer (ask-question "Quant de mal et fan els bracos del 0 (mínim) al 3 (mŕxim): " 0 1 2 3))
    (assert (fa-mal bra ?answer)))

(defrule mal-cames
    =>
    (bind ?answer (ask-question "Quant de mal et fan les cames del 0 (mínim) al 3 (mŕxim): " 0 1 2 3))
    (assert (fa-mal cam ?answer)))

(defrule mal-pit
    =>
    (bind ?answer (ask-question "Quant de mal et fan el pit del 0 (mínim) al 3 (mŕxim): " 0 1 2 3))
    (assert (fa-mal pit ?answer)))

(defrule mal-abs
    =>
    (bind ?answer (ask-question "Quant de mal et fan els abdominals del 0 (mínim) al 3 (mŕxim): " 0 1 2 3))
...
(defrule welcome
    ...
    (printout t "Benvingut al generador de programes d'entrenament, abans de res et demanem que contestis algunes preguntes per poder adaptar-te al mŕxim l'entrenament." crlf crlf)
)
